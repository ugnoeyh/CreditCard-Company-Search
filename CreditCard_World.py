...
def searchInDir(cc_path, regex_list, mask):
    ...
    신용 카드가 포함 된 튜플을 반환
    '''
    count_cc_total = 0  # 파일에서 찾은 총 신용 카드 수
    count_nocc_files = 0  # 신용 카드 번호가 없는 파일
    count_discarded_files = 0  # 지원되지 않는 파일
    count_cc_files = 0  # 신용 카드가 포함 된 파일
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Syntax: CreditCard_World.py -i </파일명> -d <inputdirectory> [-m] ')
        sys.exit(2)
    if not opts:
        print('Syntax: CreditCard_World.py -i <inputfile> -d <inputdirectory> [-m] ')
        sys.exit(2)
